...
**함수형 컴포넌트가 나온 이유는 클래스 컴포넌트의 복잡성과 로직의 재사용성에 초점을 두고 있습니다.** 리액트 컴포넌트는 Input 에 대한 render() 라는 Output 을 내는 '함수' 에 가깝습니다. 리액트 컴포넌트의 Input 과 Output 사이에 필요한 상태나 어떠한 처리가 필요하다면 React Hooks API 를 이용해 '로직'을 분리해 다른 컴포넌트에서도 재사용 가능하게 만들어 줄 수 있습니다.
